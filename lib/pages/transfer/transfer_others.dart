...
class TransferOthersScreen extends StatefulWidget {
  const TransferOthersScreen({super.key});

  @override
  _TransferOthersScreenState createState() =>
      _TransferOthersScreenState();
}

class _TransferOthersScreenState extends State<TransferOthersScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Transferência de Outros Itens'),
      ),
  ...
  Future<void> _transferItem(BuildContext context, String patrimony,
    ...
    if (_patrimonyController.text.isNotEmpty &&
      ...
      try {
        final http.Response response = await http.post(
          Uri.parse(Urls.TransferOthers.value),
          headers: <String, String>{
            'Content-Type': 'application/json',
            'Authorization': 'Bearer $token',
          },
