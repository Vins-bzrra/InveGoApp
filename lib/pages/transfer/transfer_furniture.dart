...
class TransferFurnitureScreen extends StatefulWidget {
  const TransferFurnitureScreen({super.key});

  @override
  _TransferFurnitureScreenState createState() =>
      _TransferFurnitureScreenState();
}

class _TransferFurnitureScreenState extends State<TransferFurnitureScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Transferência de Item de Mobiliário'),
      ),
  ...
  Future<void> _transferItem(BuildContext context, String patrimony,
    ...
    if (_patrimonyController.text.isNotEmpty &&
      ...
      try {
        final http.Response response = await http.post(
          Uri.parse(Urls.TranferFurniture.value),
          headers: <String, String>{
            'Content-Type': 'application/json',
            'Authorization': 'Bearer $token',
          },
